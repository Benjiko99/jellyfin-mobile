...
- **Desktop is a target, not a port.** `jvm("desktop")` builds the same `commonMain` into a Compose
  ...
  downloads it and packages it into the app (`:desktopApp:bundleVlc`, ~104 MB), the `.deb` declares
  it as a package dependency instead (`declareVlcDebDependency`), and macOS still falls back to the
  machine's own VLC — PLAN.md §6.5. **The tests always use the machine's VLC**, because a test JVM
  has no packaged app resources to look in, so `VlcjPlayerEngineTest` skips itself where VLC is not
  installed.
